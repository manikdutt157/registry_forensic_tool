...
<head>
  <meta charset="utf-8">
  <title>Registry Forensic Report - {case_name}</title>
  <style>
    :root {{
      --bg: #f5f7fb;
      --panel: #ffffff;
...
<body>
  <header>
    <h1>Registry Forensic Report</h1>
...
def generate_pdf_report(case_dir, artifacts=REPORT_ARTIFACTS):
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=1.4 * cm,
        bottomMargin=1.4 * cm,
        title=f"Registry Forensic Report - {case_dir.name}",
    )

    story = []
    story.append(Paragraph("Registry Forensic Report", title_style))
    ...
    story.append(Paragraph("Report Summary", h2_style))
...
def generate_docx_report(case_dir, artifacts=REPORT_ARTIFACTS):
    ...
    doc = Document()
    doc.add_heading("Registry Forensic Report", level=0)
...
def generate_xlsx_report(case_dir, artifacts=REPORT_ARTIFACTS):
    """
    Generate an XLSX workbook under <case_dir>/Reports using only the standard library.
    """
    case_dir = Path(case_dir)
    report_dir = case_dir / "Reports"
    report_dir.mkdir(parents=True, exist_ok=True)
    report_path = report_dir / f"registry_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"

    summary_rows, report_sections = collect_report_data(case_dir, artifacts)

    sheets = [("Summary", [["Category", "Rows", "Source"]] + [[t, c, s] for t, c, s in summary_rows])]
    used_names = {"Summary"}
    for section in report_sections:
        title = _safe_sheet_name(str(section["title"]), used_names)
        status = str(section["status"])
        if status == "missing":
            rows = [["Status", "Source"], ["Missing", str(section["source"])]]
        else:
            headers: List[str] = list(section["headers"])  # type: ignore[assignment]
            data_rows: List[List[str]] = [list(r) for r in section["rows"]]  # type: ignore[assignment]
            rows = [headers] + data_rows if headers else [["Status"], ["No columns found"]]
        sheets.append((title, rows))

    with zipfile.ZipFile(report_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        _write_xlsx_package(archive, sheets)
    return report_path
...
def _safe_sheet_name(name, used_names):
    ...
    while safe in used_names:
        suffix = f" {index}"
        safe = (base[: 31 - len(suffix)] + suffix).strip()
        index += 1
    used_names.add(safe)
    return safe
...
def _write_xlsx_package(archive, sheets):
    archive.writestr(
        "[Content_Types].xml",
        '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
...
def generate_reports(case_dir, formats: Iterable[str] = ("pdf", "docx")) -> Dict[str, Path]:
    ...
    for fmt in formats:
        ...
        elif f in ("excel", "xls", "xlsx"):
            outputs["xlsx"] = Path(generate_xlsx_report(case_dir))
        else:
